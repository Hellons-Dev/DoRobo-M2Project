...
Point multi_obs(Obstacle* thisAngle, int nbr_obs)
{
    ...
    //double adjustedNvAngle = 0.0;
    while (i != nbr_obs){

		if (thisAngle[i].EndAngle - thisAngle[i+1].StartAngle < -35){

            nv_angle = thisAngle[i].EndAngle + 20;
            nv_angle = nv_angle * pi / 180;
            new_coord.y = y - (sin(adjustedAngle) * (thisAngle[i].Distance));
            new_coord.x = x + (cos(adjustedAngle) * (thisAngle[i].Distance) * tan(nv_angle));
            pc.printf("\n\rnow distance = %d  angle = %d et %lf\n\r", thisAngle[i].Distance, nv_angle, adjustedAngle);
    ...
    if (ok == 0){
        ...
        new_coord.y = y + (sin(adjustedAngle) * 800);
        new_coord.x = x - (cos(adjustedAngle) * 800);
...
{
    int Obs_traj = 0;
    Obstacle tab_angle[360];
    Point new_coord;
    bool t1 = false, t2 = false;

    for(int i = 0; i<nbr_obs; i++){
        
        if(angle[i].StartAngle < 90 || angle[i].EndAngle >= 270){
            tab_angle[i] = angle[i];
            Obs_traj++;
        }
        if(tab_angle[i].StartAngle < 90 && tab_angle[i].EndAngle >= 90){
            t1 = true;
            tab_angle[i].EndAngle = 90;
        }
        if(tab_angle[i].StartAngle < 270 && tab_angle[i].EndAngle >= 270){
            t2 = true;
            tab_angle[i].StartAngle = 269;
        }
        pc.printf("\n\rdistance = %d  angleS = %d  angleE = %d\n\r", angle[i].Distance, angle[i].StartAngle, angle[i].EndAngle);
    }
    
    if(t1 && !t2){
        tab_angle[Obs_traj].StartAngle = 269;
        tab_angle[Obs_traj].EndAngle = 270;
        tab_angle[Obs_traj].Distance = 800;
        Obs_traj ++;
    }else if(!t1 && t2){
        for(int i = Obs_traj; i>0; i--){
            tab_angle[i] = tab_angle[i-1];
        }
        tab_angle[0].StartAngle = 89;
        tab_angle[0].EndAngle = 90;
        tab_angle[0].Distance = 800;
        Obs_traj ++;
    }else if(!t1 && !t2){
        for(int i = Obs_traj; i>0; i--){
            tab_angle[i] = tab_angle[i-1];
